...
if (acao === 'status') {
  ...
  const opts = { access: 'public', contentType: 'audio/mpeg', addRandomSuffix: true, token };
  const [i, v] = await Promise.all([
    put(`cover/resultados/${jobId}/instrumental.mp3`, await fs.readFile(a1), opts),
    put(`cover/resultados/${jobId}/voz.mp3`, await fs.readFile(a2), opts),
  ]);
