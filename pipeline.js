...
- Retorne APENAS um JSON válido, sem texto antes ou depois, no formato:
{
  ...
  "textoThumbnail": "2 a 3 palavras curtas e impactantes pra sobrepor na thumbnail, no mesmo espírito de mistério/revelação do título (ex: 'FÉ TESTADA', 'A TRAIÇÃO', 'O SEGREDO'), em maiúsculas, em português",
...
export async function publicarYoutubePrivado({ videoUrl, thumbnailUrl, titulo, descricao, tags }) {
...
export async function gerarThumbnail({ tema, titulo, estilo, textoThumbnail }) {
  ...
  } Close extremo no rosto do personagem principal com expressão forte e emocional, olhar direto pra câmera, vestido com roupas completas da época. Fundo desfocado com elemento simbólico da história. Composição de regra dos terços, alto contraste, cores saturadas e quentes. Sem texto sobreposto. Sem marca d'água. Qualidade 4K. Evite: armas, espadas, facas, sangue, ferimentos, nudez, torso nu, violência gráfica.`;
  ...
  if (!textoThumbnail || !process.env.SHOTSTACK_API_KEY) return blobBase.url;
              ...
              clips: [
                {
                  asset: {
                    ...
                    css: `p { font-family: 'Open Sans', sans-serif; font-size: 84px; font-weight: 900; color: #ffffff; text-align: center; text-shadow: 4px 4px 0 #000, -4px -4px 0 #000, 4px -4px 0 #000, -4px 4px 0 #000, 0 0 12px rgba(0,0,0,0.8); margin: 0; text-transform: uppercase; letter-spacing: 1px; }`,
                    width: 1000,
                    height: 180,
                  },
                  start: 0,
