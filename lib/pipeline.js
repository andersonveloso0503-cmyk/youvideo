...
- Retorne APENAS um JSON válido, sem texto antes ou depois, no formato:
{
  ...
  // se ficou muito curto, pede pra ela mesma continuar de onde parou em vez
  // de aceitar uma oração mais curta que o pedido.
  const contarPalavras = (t) => (t || '').trim().split(/\s+/).filter(Boolean).length;
  if (contarPalavras(parsed.narracao) < palavrasAlvo * 0.85) {
    const jaTem = contarPalavras(parsed.narracao);
    const continuacaoRes = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${process.env.GROQ_API_KEY}`,
